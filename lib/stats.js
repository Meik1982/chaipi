...
export function loadStats(statsPath = STATS_FILE) {
    ...
    try {
        ...
        // Datenabgleich & Migration: Jedes lokal verarbeitete Token spart 1:1 Cloud-Quota
        const expectedTotalSaved = (stats.totalPromptTokens || 0) + (stats.totalCompletionTokens || 0);
        if ((stats.totalSavedTokens || 0) < expectedTotalSaved) {
            stats.totalSavedTokens = expectedTotalSaved;
        }

        if (stats.daily && typeof stats.daily === 'object') {
            for (const day of Object.values(stats.daily)) {
                if (day && typeof day === 'object') {
                    const dayTotal = (day.promptTokens || 0) + (day.completionTokens || 0);
                    if ((day.savedTokens || 0) < dayTotal) {
                        day.savedTokens = dayTotal;
                    }
                }
            }
        }
...
export function recordUsage(entry, statsPath = STATS_FILE) {
    ...
    // Jedes lokal durch Gemini Nano berechnete Token schont die Cloud-Quota 1:1
    const saved = pTok + cTok;
...
export function computeStatsMetrics(stats = loadStats(), now = Date.now()) {
    ...
    const totalSavedTokens = stats.totalSavedTokens || (totalPromptTokens + totalCompletionTokens);
    const estimatedSavingsUsd = Number(((totalPromptTokens * 0.15 + totalCompletionTokens * 0.60) / 1000000).toFixed(4));
    ...
    return {
        ...
        todaySavedTokens: todayStats.savedTokens || (todayStats.promptTokens + todayStats.completionTokens),
        totalRequests: stats.totalRequests || 0,
        totalSavedTokens,
        savedRequestsToday: todayStats.requests,
        savedRequestsTotal: stats.totalRequests || 0,
        estimatedSavingsUsd,
        avgTokPerSec
    };
}
